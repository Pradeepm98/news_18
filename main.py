...
async def process_for_upcoming_match_by_id(name: str, id: str, isClub: bool):
    ...
    teamimage1x_map = {}
    for team_id, team in json_resp["teams"].items():
        teamimage1x_map[int(team_id)] = team["image1x"]
    teamimage2x_map = {}
    for team_id, team in json_resp["teams"].items():
        teamimage2x_map[int(team_id)] = team["image2x"]

    match = json_resp["matches"][0]
    start = time.time()
    result = await get_next_match_club(team_map[match["match"]["home"]])
    print('execution time in seconds',time.time() - start)

    if not result['search_result']:
        return {"name": name, "upcoming_match": "No data found"}
    upcoming_match = result['search_result'][0]

    return {
        "name": name,
        "teamname":team_map[match["match"]["home"]],
        "teamlogo": teamimage1x_map[match["match"]["home"]],
        "upcoming_match": upcoming_match['upcoming_match'],
    }
